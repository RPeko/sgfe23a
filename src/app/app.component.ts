...
export class AppComponent implements OnInit {
 ...
 listaKO: KO[] = [
  ...
  ngOnInit() {
    ...
    this.storageMap.get('srbKO').subscribe((katOpst) => {
      if (katOpst) {
        ...
        this.currentKO = ' (' + katOpst + ')';
      }
      this.storageMap.get('srbMnLayers').subscribe((ls: any) => {
        if (ls) {
          this.menuLayers = ls;
  ...
  getLayer(id: number) {
    ...
    this.listaKat.forEach(kat => {
      if (kat.layers.findIndex(l => l.id === id) > -1) {
        findLayer =  kat.layers.find(l => l.id === id);
       // console.log(JSON.stringify(layer));
      } else {
        kat.subkat.forEach(extSubkat => {
          if (extSubkat.layers.findIndex(l => l.id === id) > -1) {
            findLayer = extSubkat.layers.find(l => l.id === id);
          }
        });
      }
    });
    layer = findLayer??new Layer();
    return layer;
  }
  ...
  changeLayerPreview(ev: any, layer: Layer) {
    const x = +ev.clientX;
    const y = +ev.clientY;
