...
import { TackeService } from '../providers/tacke.service';
import { DetaljiService } from '../providers/detalji.service';
import { MLinijeService } from '../providers/m-linije.service';
import { PoligoniService } from '../providers/poligoni.service';
...
export class MapaComponent {
  ...
  loadLayer(layer: Layer) {
    ...
    switch(layer.fillPattern) {
      case 'stripes':
        layer.leaflet_fill_pattern = new (L as any).StripePattern({color: layer.fillColor});
        layer.leaflet_fill_pattern.addTo(this.mymap);
        break;
      case 'circle':
        layer.leaflet_fill_pattern = new (L as any).CirclePattern({color: layer.fillColor});
        layer.leaflet_fill_pattern.addTo(this.mymap);
        break;
    }
  }
  ...
  unloadLayer(layer: Layer) {
    ...
    if (layer.leaflet_fill_pattern){
      layer.leaflet_fill_pattern.removeFrom(this.mymap);
    }
  }
  /////////////////////////////////////////////////////////////////////////////////////////////////////
  ////// Tacke //////////////////////////////////////////////////////////////////////////////////////
    ...
    this.tackeService.getListaTacaka(layer, this.KO.idKO).subscribe(
      lista => {
        ...
        if (!layer.preserveFitToBounds && lmmZaKO.length > 0) {
          const fg = new (L as any).featureGroup(lmmZaKO);
          const bounds = fg.getBounds(); //bilo je  as L.bounds
    ...
    this.mlinijeService.getListaMLinija(layer, this.KO.idKO).subscribe(
      lista => {
        ...
        if (!layer.preserveFitToBounds && lmlZaKO.length > 0) {
          const fg = new (L as any).featureGroup(lmlZaKO);
          const bounds = fg.getBounds(); // bilo + as L.bounds
          this.mymap.flyToBounds(bounds, { padding: [15, 15] });
        }
      });
      ...
      lista => {
        if (lista) {
          lista.forEach(poligon => {
            ...
            if (layer.leaflet_fill_pattern && poligon.useFillPattern){
              lmpPoligon.setStyle({dashArray: layer.leaflet_fill_pattern});
            }
        ...
        if (!layer.preserveFitToBounds && lmpZaKO.length > 0) {
          const fg = new (L as any).featureGroup(lmpZaKO);
